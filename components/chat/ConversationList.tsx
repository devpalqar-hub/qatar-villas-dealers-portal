...
export default function ConversationList({
    ...
    const filteredConversations = conversations.filter((c) => {
        ...
        if (searchQuery.trim()) {
            ...
            const matchesName = c.user?.name ? c.user.name.toLowerCase().includes(query) : false;
            const matchesListing = c.listing?.propertyName
                ? c.listing.propertyName.toLowerCase().includes(query)
                : false;
            const matchesMsg = c.lastMessage?.content
                ? c.lastMessage.content.toLowerCase().includes(query)
                : false;
            return matchesName || matchesListing || matchesMsg;
        }
        return true;
    });
        ...
        if (error)
            return (
                <div className={styles.errorState}>
                    ...
                    <p className={styles.errorTitle}>{t("failedToLoad")}</p>
        ...
        if (filteredConversations.length === 0)
            return (
                <div className={styles.emptySearch}>
                    {searchQuery ? t("noResults", { query: searchQuery }) : t("noConversations")}
                </div>
            );
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.tabsRow}>
                ...
                >
                    {t("tabs.all")}
                    {!loading && <span className={styles.tabBadge}>{totalCount}</span>}
                </button>
                <button
                    type="button"
                ...
                >
                    {t("tabs.unread")}
                    {!loading && <span className={styles.tabBadge}>{unreadTotal}</span>}
                </button>
                <button
                    type="button"
                    className={`${styles.tabBtn} ${activeTab === "my_listings" ? styles.tabActive : ""}`}
