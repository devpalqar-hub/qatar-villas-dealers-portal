...
import { FiPlus, FiSearch, FiEye, FiImage, FiChevronLeft, FiChevronRight, FiFilter, FiX, FiRotateCcw, FiCheckCircle, FiCreditCard, FiGift } from "react-icons/fi";
import {useFormatter, useTranslations} from "next-intl";
import { AppLayout, Button, Badge, ConfirmModal } from "@/components/ui";
import { SubscriptionEntitlementModal } from "@/components/subscription";
...
const PROPERTY_STATUSES = ["PENDING", "ACTIVE", "INACTIVE", "RESUBMITED", "REJECTED", "SOLD", "PENDING_PAYMENT"];
...
export default function PropertiesPage() {
    ...
    const [payingPropertyId, setPayingPropertyId] = useState<string | null>(null);
    const [entitlementModalProperty, setEntitlementModalProperty] = useState<PropertyListing | null>(null);
    ...
    const proceedWithPayment = async (property: PropertyListing) => {
        if (payingPropertyId) return;
        setActionError(null); setActionNotice(null); setPayingPropertyId(property.id);
        try {
            ...
            if (result.activated) {
                setPropertiesList((current) => current.map((p) => p.id === property.id ? { ...p, status: "ACTIVE" } : p));
                setActionNotice(t("property.makePaymentActivated"));
                void entitlements.refresh();
            } else if (result.paymentUrl) {
                window.location.assign(result.paymentUrl);
                return;
            } else {
                setActionError(t("property.makePaymentError"));
            }
        } catch (error: unknown) {
            const serviceError = error as { response?: { data?: { message?: string } } };
            setActionError(serviceError.response?.data?.message || t("property.makePaymentError"));
        } finally { setPayingPropertyId(null); }
    };

    // If we're confident there's no free-listing entitlement left, let the dealer choose
    // between paying individually or reviewing subscription plans before charging them.
    // Otherwise (entitlement available, or we simply don't know yet) proceed as before —
    // the backend itself decides whether the payment is free or paid.
    const handlePayClick = (property: PropertyListing) => {
        if (!entitlements.loading && !entitlements.canUseFreeListing) {
            setEntitlementModalProperty(property);
            return;
        }
        void proceedWithPayment(property);
    };

    const handlePayIndividuallyFromModal = () => {
        if (!entitlementModalProperty) return;
        const property = entitlementModalProperty;
        setEntitlementModalProperty(null);
        void proceedWithPayment(property);
    };

    const handlePlanSelectedFromModal = async (plan: SubscriptionPlan) => {
        if (checkoutPlanId) return;
        setCheckoutPlanId(plan.id);
        try {
            const res = await subscriptionService.checkout(plan.id);
            if (res.url) {
    ...
    const getStatusVariant = (status: string) => status?.toUpperCase() === "ACTIVE" ? "success" : status?.toUpperCase() === "PENDING" || status?.toUpperCase() === "RESUBMITED" || status?.toUpperCase() === "PENDING_PAYMENT" ? "warning" : status?.toUpperCase() === "REJECTED" ? "danger" : status?.toUpperCase() === "SOLD" ? "info" : "default";
    ...
    return (
        <AppLayout>
            <div className={styles.container}>
                ...
                {actionNotice && <div className={styles.actionNoticeBanner}>{actionNotice}</div>}
                ...
                <div className={styles.tableContainer}><div className={styles.tableWrapper}><table className={styles.table}><thead><tr><th className={styles.th}>{t("property.thumbnail")}</th><th className={styles.th}>{t("property.name")}</th><th className={styles.th}>{t("property.type")}</th><th className={styles.th}>{t("property.price")}</th><th className={styles.th}>{t("property.location")}</th><th className={styles.th}>{t("property.status")}</th><th className={styles.th} style={{ textAlign: "right" }}>{t("property.actions")}</th></tr></thead><tbody>{loading ? Array.from({ length: 5 }).map((_, i) => <tr key={`skeleton-${i}`}><td className={styles.td}><div className={styles.skeletonBlock} style={{ width: 64, height: 48, borderRadius: 6 }} /></td><td className={styles.td}><div style={{ display: "flex", flexDirection: "column", gap: 6 }}><div className={styles.skeletonBlock} style={{ width: 140, height: 16 }} /><div className={styles.skeletonBlock} style={{ width: 80, height: 12 }} /></div></td><td className={styles.td}><div className={styles.skeletonBlock} style={{ width: 70, height: 22 }} /></td><td className={styles.td}><div className={styles.skeletonBlock} style={{ width: 90, height: 16 }} /></td><td className={styles.td}><div style={{ display: "flex", flexDirection: "column", gap: 6 }}><div className={styles.skeletonBlock} style={{ width: 100, height: 16 }} /><div className={styles.skeletonBlock} style={{ width: 80, height: 12 }} /></div></td><td className={styles.td}><div className={styles.skeletonBlock} style={{ width: 60, height: 22 }} /></td><td className={styles.td} style={{ textAlign: "right" }}><div className={styles.actionsCell}><div className={styles.skeletonBlock} style={{ width: 32, height: 32, borderRadius: "50%" }} /><div className={styles.skeletonBlock} style={{ width: 32, height: 32, borderRadius: "50%" }} /></div></td></tr>) : displayedProperties.length === 0 ? <tr><td colSpan={7} className={styles.emptyState}>{t("property.empty")}</td></tr> : displayedProperties.map((prop) => { const isSold = prop.status?.toUpperCase() === "SOLD"; const isSelling = sellingPropertyId === prop.id; const isPendingPayment = prop.status?.toUpperCase() === "PENDING_PAYMENT"; const isPaying = payingPropertyId === prop.id; return <tr key={prop.id} className={styles.tr}><td className={styles.td}>{prop.photos && prop.photos.length > 0 ? <img src={prop.photos[0].url} alt={prop.propertyName} className={styles.thumbnail} /> : <div className={styles.thumbnail} style={{ display: "flex", alignItems: "center", justifyContent: "center", color: "#9ca3af" }}><FiImage size={24} /></div>}</td><td className={styles.td}><div className={styles.propName}>{prop.propertyName}</div><div className={styles.propLocation}>REF: {prop.id.substring(0, 8)}</div></td><td className={styles.td}><span className={styles.typeBadge}>{prop.type?.title}</span></td><td className={styles.td}><span className={styles.price}>{format.number(prop.price)}</span></td><td className={styles.td}><div className={styles.propName}>{prop.areaName}</div><div className={styles.propLocation}>{prop.municipality?.name}</div></td><td className={styles.td}><Badge variant={getStatusVariant(prop.status)}>{t(`statusEnum.${prop.status.toUpperCase()}`)}</Badge></td><td className={styles.td} style={{ textAlign: "right" }}><div className={styles.actionsCell}><button className={styles.actionBtn} title={t("property.viewProperty")} onClick={() => router.push(`/properties/${prop.id}`)}><FiEye size={18} /></button><button className={`${styles.actionBtn} ${styles.soldActionBtn} ${isSold ? styles.actionBtnDisabled : ""}`} title={isSold ? t("property.alreadySold") : t("property.markSold")} onClick={() => handleOpenMarkSold(prop)} disabled={isSold || isSelling}><FiCheckCircle size={18} /></button>{isPendingPayment && <button className={`${styles.actionBtn} ${styles.payActionBtn}`} title={!entitlements.loading && entitlements.canUseFreeListing ? t("property.freeListingAvailable") : t("property.makePayment")} onClick={() => handlePayClick(prop)} disabled={isPaying}>{!entitlements.loading && entitlements.canUseFreeListing ? <FiGift size={18} /> : <FiCreditCard size={18} />}</button>}</div></td></tr>; })}</tbody></table></div>
            ...
            <SubscriptionEntitlementModal
                open={Boolean(entitlementModalProperty)}
                context="LISTING"
                reason={entitlements.hasActiveSubscription ? "LIMIT_REACHED" : "NO_SUBSCRIPTION"}
                currentPlanId={entitlements.currentSubscription?.planId}
                onClose={() => { if (!payingPropertyId && !checkoutPlanId) setEntitlementModalProperty(null); }}
                onPayIndividually={handlePayIndividuallyFromModal}
                payIndividuallyLoading={!!payingPropertyId}
                onPlanSelected={(plan) => void handlePlanSelectedFromModal(plan)}
